...
import * as svc from '../services/crmInteraction.service.js';
...
export async function list(req, res) {
  ...
  catch (err) { return handleError(res, err, 'Could not load interactions'); }
}

export async function getById(req, res) {
  try { return res.json(await svc.getById(pool, req.params.id, req.authStaff)); }
  catch (err) { return handleError(res, err, 'Could not load interaction'); }
}

export async function create(req, res) {
  try { return res.status(201).json(await svc.create(pool, req.body, req.authStaff)); }
  catch (err) { return handleError(res, err, 'Could not create interaction'); }
}

export async function update(req, res) {
  try { return res.json(await svc.update(pool, req.params.id, req.body, req.authStaff)); }
  catch (err) { return handleError(res, err, 'Could not update interaction'); }
}

export async function remove(req, res) {
  try { return res.json(await svc.remove(pool, req.params.id, req.authStaff)); }
  catch (err) { return handleError(res, err, 'Could not delete interaction'); }
}
