...
export async function upsertEnrollment(pool, { deviceToken, companyId, branchId, label }) {
  await pool.query(
    `INSERT INTO core.pos_device_enrollment
       (device_token, company_id, branch_id, counter_no, label, enrolled_at, last_seen_at, record_status)
     VALUES ($1, $2, $3, 1, $4, NOW(), NOW(), 'ACTIVE')
...
export async function findByToken(pool, deviceToken) {
  const { rows } = await pool.query(
    `SELECT company_id, branch_id, COALESCE(counter_no, 1) AS counter_no, label
