...
/**
 * Staff picker list for the login screen: id + name only, NO pin/hash.
 * Only staff that actually have a PIN set (i.e. can sign in to Counter-POS).
 */
export async function listActiveStaffForPicker(pool, companyId) {
  const { rows } = await pool.query(
    `SELECT s.id, s.staff_id, s.staff_name, s.staff_code, r.role_name,
            r.software_type AS role_software_type
     FROM core.staff_master s
     LEFT JOIN core.role_master r ON r.company_id = s.company_id AND r.role_id = s.role_id
     WHERE s.company_id = $1
...
/** Single active staff (by internal id) WITH pin hash — for targeted PIN verify. */
export async function findActiveStaffByIdWithPin(pool, companyId, staffPk) {
  const { rows } = await pool.query(
    `SELECT ${STAFF_SESSION_COLS}, s.staff_pin
     ${SESSION_JOINS}
     WHERE s.company_id = $1
...
/** Load all active staff with their PIN hashes for PIN-only login (legacy fallback). */
