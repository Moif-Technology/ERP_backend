...
export async function insertSalesMaster(client, row) {
  ...
  await client.query(
    ...
    [
      ...
      Math.abs(Number(cashAmount) || 0),
      Math.abs(Number(creditAmount) || 0),
      Math.abs(Number(creditCardAmount) || 0),
      Math.abs(Number(paidAmount) || 0),
      Math.abs(Number(balancePaid) || 0),
      Math.abs(Number(discountAmount) || 0),
      Math.abs(Number(subtotalAmount) || 0),
      Math.abs(Number(taxableAmount) || 0),
      Math.abs(Number(tax1Amount) || 0),
      Math.abs(Number(tax2Amount) || 0),
      Math.abs(Number(tax3Amount) || 0),
      tax1Rate,
      tax2Rate,
      tax3Rate,
      roundOffAdj,
...
export async function insertSalesChild(client, row) {
  ...
  await client.query(
    ...
    [
      ...
      Math.abs(Number(unitPrice) || 0),
      Math.abs(Number(unitCost) || 0),
      Math.abs(Number(packQty) || 1) || 1,
      Math.abs(Number(discountAmount) || 0),
      Math.abs(Number(lineTotal) || 0),
      Math.abs(Number(tax1Amount) || 0),
      Math.abs(Number(tax2Amount) || 0),
      Math.abs(Number(tax3Amount) || 0),
      tax1Rate,
...
export async function insertSalesPaymentSplit(client, row) {
  ...
  await client.query(
    ...
    [companyId, salesId, payerNo, payMode, Math.abs(Number(billAmount) || 0), branchId, counter, staffId, refNo]
  );
}
