-- 112_salon_specific_features.sql
-- Add salon-specific feature toggles for SalonPOS buttons
-- These are granular controls for individual salon features, separate from POS

BEGIN;

-- Insert salon-specific features
INSERT INTO core.feature_master
  (feature_code, feature_name, pack_code, parent_feature_code, feature_type, sort_order, is_active)
VALUES
  ('salon.jobs',              'Salon Jobs',                 'pos', 'salon', 'feature', 1, TRUE),
  ('salon.stylists',          'Salon Stylists',             'pos', 'salon', 'feature', 2, TRUE),
  ('salon.appointments',      'Salon Appointments',         'pos', 'salon', 'feature', 3, TRUE),
  ('salon.service_status',    'Salon Service Status',       'pos', 'salon', 'feature', 4, TRUE),
  ('salon.stylist_reassign',  'Salon Stylist Reassign',     'pos', 'salon', 'feature', 5, TRUE)
...
WHERE rm.role_id = 1
  AND pm.feature_code IN ('salon.jobs', 'salon.stylists', 'salon.appointments', 'salon.service_status', 'salon.stylist_reassign')
